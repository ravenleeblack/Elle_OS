...
{
    if (!video_memory || 
        pos_x < 0 || pos_x >= screen_width ||
        pos_y < 0 || pos_y >= screen_height) {
        return;
    }

    unsigned char* screen = video_memory;  // Get pointer to video memory
    unsigned int offset = (pos_y * screen_width + pos_x) * 4;

    screen[offset] = color.blue;
    screen[offset + 1] = color.green;
    screen[offset + 2] = color.red;
    screen[offset + 3] = color.alpha;
}
